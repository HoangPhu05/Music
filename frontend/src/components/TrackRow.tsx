...
export default function TrackRow({ song, index, allSongs, onDelete, onAddToPlaylist }: Props) {
    const { currentIndex, queue, setQueue, playSong, isPlaying } = usePlayerStore();
    ...
    const handleDownload = () => {
        ...
        // Tạo thẻ <a> ẩn để trigger download với Authorization header qua fetch
    ...
    return (
        <div
            className={`flex items-center gap-3 px-4 py-3 rounded-xl group transition-all duration-200 cursor-pointer
        ...
        >
            ...
            <div className="w-7 text-center flex-shrink-0">
                {isCurrentSong && isPlaying ? (
                    ...
                    <span className={`text-sm ${isCurrentSong ? 'text-brand-400' : 'text-gray-500 group-hover:text-gray-300'}`}>
                        {index + 1}
                    </span>
                )}
            </div>
            ...
            <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500 truncate">
                    {[song.artist, song.album].filter(Boolean).join(' • ') || 'Không rõ nghệ sĩ'}
                </p>
            </div>
            ...
            >
                ...
                <button
                    ...
                    className="p-1.5 rounded-lg hover:bg-brand-600/20 text-gray-400 hover:text-brand-300 transition-colors"
                    title="Tải xuống"
                >
                    ⬇️
                </button>
